...
export default function ManageSeriesTab() {
  ...
  const [newEpisode, setNewEpisode] = useState<Partial<Episode>>({
    title: '', episodeNumber: 1, contentType: 'json', pdfPath: '', jsonPath: ''
  });
  ...
  const openAddEpisodeModal = () => {
    ...
    setNewEpisode({ title: '', episodeNumber: (activeSeries?.episodes?.length || 0) + 1, contentType: 'json', pdfPath: '', jsonPath: '' });
    setPdfFile(null);
    setJsonFile(null);
          ...
          <div className="glass-card modal-dialog">
            ...
            <form onSubmit={handleSeriesSubmit} className="modal-form">
              <div className="form-group">
                ...
                <input type="text" className="form-input" required value={newSeries.title} onChange={e => setNewSeries({...newSeries, title: e.target.value})} />
              </div>
              <div className="form-group">
                <label className="form-label">Description</label>
                <textarea className="form-input" rows={3} required value={newSeries.description} onChange={e => setNewSeries({...newSeries, description: e.target.value})} />
              </div>
              <div className="form-group">
                <label className="form-label">Price (₦)</label>
                <input type="number" className="form-input" step="0.01" required value={newSeries.price} onChange={e => setNewSeries({...newSeries, price: Number(e.target.value)})} />
              </div>
              <div className="form-group">
                <label className="form-label">Cover Image</label>
                {imageUploadType === 'file' ? (
                  <input type="file" accept="image/*" className="form-input" onChange={e => e.target.files && setCoverFile(e.target.files[0])} />
                ) : (
                  <input type="url" className="form-input" value={newSeries.coverUrl || ''} onChange={e => setNewSeries({...newSeries, coverUrl: e.target.value})} />
                )}
          ...
          <div className="glass-card modal-dialog">
            ...
            <form onSubmit={handleEpisodeSubmit} className="modal-form">
              <div className="form-group">
                ...
                <input type="number" className="form-input" required value={newEpisode.episodeNumber} onChange={e => setNewEpisode({...newEpisode, episodeNumber: Number(e.target.value)})} />
              </div>
              <div className="form-group">
                <label className="form-label">Title</label>
                <input type="text" className="form-input" required value={newEpisode.title} onChange={e => setNewEpisode({...newEpisode, title: e.target.value})} />
              </div>
              <div className="form-group">
                <label className="form-label">Format</label>
                <select className="form-input" value={newEpisode.contentType} onChange={e => setNewEpisode({...newEpisode, contentType: e.target.value as 'pdf'|'json'})}>
              ...
              <div className="form-group">
                ...
                {newEpisode.contentType === 'pdf' ? (
                  <input type="file" accept="application/pdf" className="form-input" onChange={e => e.target.files && setPdfFile(e.target.files[0])} />
                ) : (
                  <input type="file" accept=".json,application/json" className="form-input" onChange={e => e.target.files && setJsonFile(e.target.files[0])} />
                )}
