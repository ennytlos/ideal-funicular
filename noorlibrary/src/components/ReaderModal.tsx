  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      ...
      try {
        ...
        if (active) {
          setPdfError('Failed to fetch secure book content.');
        }
      } finally {
        if (active) {
        ...
        >
          ...
          >
            ...
            {isTextBook ? (
              ...
              >
                {loadingPdf && <p style={{ fontFamily: 'Outfit, sans-serif' }}>Retrieving secure reader connection...</p>}
                {pdfError && <p style={{ color: 'var(--accent-red)', fontFamily: 'Outfit, sans-serif' }}>{pdfError}</p>}
                {pdfUrl && (
                  <Document
                    file={pdfUrl}
                    onLoadSuccess={({ numPages }) => setNumPages(numPages)}
                    loading={<p style={{ fontFamily: 'Outfit, sans-serif' }}>Decrypting book pages...</p>}
                  ...
                  >
                    ...
                    >
                      <Page 
                        ...
                        width={Math.min(600, typeof window !== 'undefined' ? window.innerWidth - 32 : 500)} 
                        scale={pdfScale}
