...
export const logFirebaseEvent = (eventName: string, eventParams?: Record<string, unknown>) => {
  if (typeof window !== 'undefined' && analytics) {
    if (!eventParams) {
      ...
      // logEvent expects params with flexible types; cast via unknown-to-Record to avoid explicit `any`
      logEvent(analytics, eventName, eventParams as Record<string, unknown>);
    }
  }
};
